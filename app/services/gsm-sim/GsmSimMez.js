import { View, Text, Dimensions, Image, ScrollView, StyleSheet, Alert } from 'react-native'
...
//   ];
    ...
    const navigateToScreenA = () => {
        const dataToPass = {
            parameter1: isMez,
            parameter2: isPreviewMez
        };
        navigation.navigate('GsmSim', { params: dataToPass });
    };
    ...
    const navBack = async () => {
        ...
        navigateToScreenA();
        params.params.hasMez === "false" ? 
        await mezPreview.mezPreview(params.params.register, params.params.lname, params.params.fname, signaturePath , params.params.foreigner).then((res) => {
            setIsPreviewMez(res.data.contractpath)
        }):null
        Alert.alert("Мэдээлэллийн эзний гэрээ хүлээн зөвшөөрлөө")
    }
    ...
    const close = async () => {
        ...
        await signatureUpload.signatureUploadFile({ base64: isSignatureBase, type: "string" }).then((res) => {
            ...
            Alert.alert("Мэдээлэллийн эзний гэрээ амжилттай хадгалагдлаа")
        })
    }
        ...
        >
            <>
                ...
                <View style={{paddingBottom: 20, padding: 10}}>
                    <AppButton title='Гарын үсэг зурах' onPress={() => setModalVisible(true)} />
                    <AppButton onPress={navBack} title='Үргэлжлүүлэх' style={Styles.bgRed} />
                </View>
            </>
        </ScrollView>
    )
